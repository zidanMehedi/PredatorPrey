...
myJSONS = [file for file in os.listdir('..\\'+sample+'\\JSON_Files')]
myImages = [img for img in os.listdir('..\\'+sample+'\\Frames')]

for img,file in zip(myImages,myJSONS):
    imageFrame = cv2.imread(os.path.join('..\\'+sample+'\\Frames',img),1)
    DataJSON = json.loads(open(os.path.join('..\\'+sample+'\\JSON_Files',file),'r').read())
    ...
    myJSON = json.loads(open(os.path.join('E:\\PredatorPrey\\'+sample+'\\JSON_Files',myJSONS[0]),'r').read())
    ...
    for data in tempJSON[1:]:
            myJSON = json.loads(open(os.path.join('..\\'+sample+'\\JSON_Files',data),'r').read())
            pdx2 = math.ceil((myJSON['shapes'][0]['points'][0][0]+myJSON['shapes'][0]['points'][1][0])/2)
            pdy2 = math.ceil((myJSON['shapes'][0]['points'][0][1]+myJSON['shapes'][0]['points'][1][1])/2)
            pdx1 = pdx
            pdy1 = pdy
